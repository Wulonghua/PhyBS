...
void DescentOptimize::initialization()
{
	...
	m_posk = m_tetMesh->getNodes() + m_h * m_tetMesh->getVelocities() + 0.2 * (m_tetMesh->getVelocities()-m_vel_old);
}

Eigen::MatrixXf DescentOptimize::computeGradient()
{
	Eigen::MatrixXf g = m_posk - m_tetMesh->getNodes() - m_h*m_tetMesh->getVelocities();
